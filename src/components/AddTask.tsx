...
interface AddTaskProps {
  onTaskAdded?: (task: { requirements: string; fileName: string }) => void;
}

function AddTask({ onTaskAdded }: AddTaskProps): JSX.Element {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    console.log({ requirements, uploadedFile });
    ...
    <>
      ...
      <div className="w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-2xl bg-white p-6 rounded-2xl shadow-lg font-mulish">
        ...
        <form
          ...
          className="flex flex-col gap-2 w-10/12 m-auto"
        >
          ...
          <div className="mt-4">
            ...
            >
              ...
              <p className="text-blue-900 text-lg sm:text-2xl">Subir Archivo</p>
            </label>
          </div>
          ...
          <button
            type="submit"
            className="bg-blue-400 w-full sm:w-2/5 text-white font-mulish font-semibold py-2 px-4 rounded-lg shadow-md shadow-gray-400 transition hover:opacity-60 active:opacity-60 active:shadow-none m-auto cursor-pointer mt-4"
          >
            Subir tarea
          </button>
        </form>
